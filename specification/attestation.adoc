...
TSM can use up to 17 runtime measurement registers for that purpose.
...
The TVM runtime measurements Claim is optional, all other above described Claims
...
for the TVM. The list must contain at most 17 entries.
...
tvm-runtime-measurements = (
   tvm-runtime-measurements-label => [ 1*17 riscv-cove-measurement ]
)
----
