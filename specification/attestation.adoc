...
evidence  attest to the trustworthiness of the TVM. When valid relying party 
releases secrets or attestation result tokens back to the trusted workload.
...
In this section we describe the high level attestation remote model for CoVE,
together with the attestation evidence content, format and generation process.

[note]
====
Question: Should  local attestation's  mechanism and protocol between the attester (i.e.
the TVM) and the TSM be out of scope for this document.  
====
