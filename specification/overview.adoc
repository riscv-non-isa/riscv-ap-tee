...
This specification describes the CoVE architecture which describes a new class of
...
[title= "TEE TCB for VM workloads"]
image::img_0.png[]
...
traditional untrusted resources in the non-confidential world, and are transitioned 
...
(TEEI)* ABI hosted by a TEE Security Manager Driver ( *TSM Driver* ) 
...
As shown in figure 1, the M-mode firmware is in the TCB of all CoVE
...
and integrity suitable for use for CoVE components. The TEEI operations for
...
provides an ABI to the OS/VMM which has two aspects: A set of host ABIs known as *COVH*
that includes functions to manage the lifecycle of the TVM, such as creating, adding pages 
to a TVM, scheduling a TVM for execution, etc. in an OS/platform agnostic 
manner. The TSM also provides an ABI to the TVM contexts: A set of guest ABIs known as 
*COVG* that enablse the TVM workload to request attestation functions, memory management  
...
an *Confidential mode bit* maintained per hart. This mode is enabled per-hart via 
TEECALL and disabled via TEERET for operations described in the TEEI. Access 
to TEE-assigned memory is allowed for the hart when the Confidential mode is set. 
This per-hart Confidential mode bit is used by the processor to enforce 
...
caches etc). The implementation details of this mode bit is out of scope for
this specification. 
...
appropriate extensions of the COVG.
...
is to propose a reference architecture and ratify a normative COVE as a RISC-V
