...
hosting supervisor domain which operates in S/HS mode. This model utilizes
...
memory to the Confidential supervisor domain.  In this model memory assigned to a Confidential
supervisor domain is called *Confidential* memory and
memory accessible to the hosting supervisor domain called *Non-Confidential*.
The MTT allows dynamic programming of the per-domain access permissions. This model
is shown in <<dep1>>
...
This model might be suitable for client/embedded systems and is shown in <<dep3>>. This model
does not require Smmtt.
...
partitioning cccurs during runtime.
dynamic partitioning allows for better resource utilization at the cost of more complexity. 
...
[Note]
====
When Smmtt is not available  in <dep3>  isolation is managed using G-Stage PTpage and
PMPs.
==== 
