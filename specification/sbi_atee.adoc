...
. `sbi_tee_host_tvm_invalidate_pages()`
. `tee_host_tvm_initiate_fence()`
. `sbi_tee_host_tvm_remove_pages()`
...
with `sbi_tee_host_run_tvm_vcpu()` will fail. Any guest page faults taken by other TVM vCPUs in the
...
[#sbi_tee_host_tvm_invalidate_pages]
=== Function: TEE Host TVM Invalidate Pages (FID #16)
[source, C]
-----
struct sbiret sbi_tee_host_tvm_invalidate_pages(unsigned long tvm_guest_id,
                                                unsigned long gpa,
...
must complete a TVM TLB invalidation sequence, initiated by `sbi_tee_host_tvm_fence()`, in
...
[#table_sbi_tee_host_tvm_invalidate_pages_errors]
.TEE Host TVM Invalidate Pages
[cols="2,3", width=90%, align="center", options="header"]
|===
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
...
[#sbi_tee_host_tvm_validate_pages]
=== Function: TEE Host TVM Validate Pages (FID #17)
[source, C]
-----
struct sbiret sbi_tee_host_tvm_validate_pages(unsigned long tvm_guest_id,
                                             unsigned long gpa,
                                             unsigned long length);
-----
...
`sbi_tee_host_tvm_invalidate_pages()`. After verifying the TSM will mark the pages as present and 
...
[#table_sbi_tee_host_tvm_validate_pages_errors]
.TEE Host TVM Validate Pages
[cols="2,3", width=90%, align="center", options="header"]
|===
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
...
[#sbi_tee_host_tvm_remove_pages]
=== Function: TEE Host TVM Remove Pages (FID #18)
[source, C]
-----
struct sbiret sbi_tee_host_tvm_remove_pages(unsigned long tvm_guest_id,
                                           unsigned long gpa,
                                           unsigned long length);
-----
...
[#table_sbi_tee_host_tvm_remove_pages_errors]
.TEE Host TVM Remove Pages
[cols="2,3", width=90%, align="center", options="header"]
|===
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
...
| <<sbi_tee_host_tvm_invalidate_pages, sbi_tee_host_tvm_invalidate_pages>> |
Invalidates the pages in the specified range of guest physical address space and thus marks the
...
| <<sbi_tee_host_tvm_validate_pages, sbi_tee_host_tvm_validate_pages>> |
...
| <<sbi_tee_host_tvm_remove_pages, sbi_tee_host_tvm_remove_pages>> |
