...
`sbi_tee_host_run_tvm_vcpu()` to begin execution. The host must boot vCPU `0` first otherwise
`sbi_tee_host_run_tvm_vcpu()` call will fail. TVM execution continues until there an event like an
...
is destroyed. This call must not be made after calling `sbi_tee_host_finalize_tvm()`.
