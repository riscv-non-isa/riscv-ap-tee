...
(2) the OS/VMM assembles a TVM under the TSM's supervision in a serie of COVH calls. 
...
(via `sbi_covh_promote_to_tvm()`) because TSM creates all TVM's vCPUs in one operation.
TVM's vCPUs are in the reset state, except for the one whose state was presented to TSM
at the time of promotion and included in the TVM's measurement.
====
...
models that require single-step TVM creation (e.g., see <<dep3>>). Deployment models that offer multi-step TVM creation 
...
[NOTE]
====
To simplify the implementation of this ABI, it is recommended that this call is already invoked by the VM and then only reflected 
to the TSM by the OS/VMM. The advantage is that the OS/VMM does not need any specific mechanism for creating a TVM because it can
construct a TVM as a regular VM and rely on the VM to select the time to promote it to TVM.
====

If successful, the TSM sets the TVM state to `TVM_RUNNABLE` and returns a unique TVM identifier (`tvm_guest_id`) to the OS/VMM, 
which should free the contents of non-confidential memory containing VM's data and page tables. Since now on, the OS/VMM must 
...
presented to TSM during the TVM creation via `sbi_covh_promote_to_tvm()`. Only TVMs correctly 
