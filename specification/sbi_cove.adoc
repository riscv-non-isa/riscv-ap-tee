...
(1) the OS/VMM requests the TSM to promote an existing VM to a TVM using a single COVH call, 
(2) the OS/VMM assembles a TVM under the TSM's supervision in a sequence of COVH calls. 

To promote a non-confidential VM to a TVM, the OS/VMM invokes the `sbi_covh_promote_to_tvm()` call,
presenting the VM state (the CPU state, VM's data, page tables) to the TSM. The TSM handles this call 
by copying the entire VMs state into confidential memory. This is performed as a single operation after 
which the newly created TVM is in the `TVM_RUNNABLE` state. 
...
(via `sbi_covh_promote_to_tvm()`) because TSM creates all TVM's vCPUs at once.
All TVM's vCPUs are in the reset state, except for the vCPU whose state was presented to the TSM
at the time of promotion. The vCPUs initial states are part of the TVM's measurement.
====
...
[NOTE]
====
Note that a TVM created via `sbi_covh_promote_to_tvm()` does not require finalization, 
because it is already in the `TVM_RUNNABLE` state with a valid entrypoint, which
corresponds to the vCPU state presented to TSM at the time of promotion to a TVM.
====
...
struct sbiret sbi_covh_promote_to_tvm(unsigned long fdt_addr,
                                      unsigned long number_of_vcpus,
                                      unsigned long tap_addr);
-----
This intrisic is used by the host to promote a VM to a TVM. It is primarily inteded for CoVE deployment 
models that require the single-step TVM creation (e.g., see <<dep3>>). Deployment models that offer the multi-step TVM creation 
(e.g., <<dep1>>) may, but are not required to, support this ABI as an additional mechanism for creating TVMs. 

The `fdt_addr` is the 8-bytes aligned guest physical address of the flattened device tree (FDT). The `fdt_addr` is 
a number representing a maximum number of vCPUs that this TVM can have. The `tap_addr` is the 8-bytes aligned guest physical 
address of the `TVM attestation payload` used for local attestation. For VMs that do not require local attestation 
(only the remote attestation), `tap_addr` must be `0`.

The VM should be promoted as early in the boot process as possible to minimize changes in contents of memory so 
that the resulting integrity measurement hashes are deterministic. The state of the processor (i.e., GPRs and VS-level CSRs) 
that trapped to TSM with this call, must correspond to the state of the vCPU that will resume execution as part of the TVM. 
The TSM copies VM's data to confidential memory, while recreating page table hierarchy in confidential memory following
the VM page table configuration stored in non-confidential memory and pointed by the HGATP. The TSM populates one of the vCPUs 
state with the state of the trapped thread, all other vCPUs are in the reset state and controlled by the TVM using the SBI HSM
extension. TSM measures the initial TVM's state and uses during local or remote attestation.

[NOTE]
====
To simplify the implementation of this ABI, it is recommended that this call is already invoked by the VM and then OS/VMM just reflects 
it to the TSM. The advantage is that the OS/VMM does not need to implement any specific mechanism for creating a TVM because it can
construct a TVM as a regular VM, relying on the VM to select the time of promotion to a TVM.
====

If successful, the TSM sets the TVM state to `TVM_RUNNABLE` and returns a unique TVM identifier (`tvm_guest_id`) to the OS/VMM. 
The OS/VMM should free the contents of non-confidential memory that contains VM's data and page tables. Since now on, the OS/VMM must 
interact with this TVM via the TSM using the COVH ABI, i.e., resuming the TVM using the `sbi_covh_run_tvm_vcpu()` call.

If the call fails, the TSM returns the SBI error code in `sbiret.error` to the OS/VMM. The possible error codes 
are shown below.
...
presented to TSM during the TVM creation via `sbi_covh_promote_to_tvm()`. Only the TVMs that were correctly 
authenticated and authorized during local attestation can receive the secret embedded in TAP.

This ABI will become part of the `Sealing Interface` planned for the CoVE in version 2.0.
