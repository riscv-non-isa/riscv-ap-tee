...
to a TVM, (2) VMM assemblies a TVM under the TSM supervision.

The promotion of a non-confidential VM to a TVM happens when a non-confidential VM
executes a `sbi_covh_create_tvm()` call. TSM handles this call by copying the entire
VMs state (CPU state, memory) into confidential memory. This is performed as a single 
operation after which the newly created TSM is in the `TVM_RUNNABLE` state. This way
of TVM creation reduces the number of ABI calls, thus simplifies the TSM implementation.
...
The advantage of this approach is that the TVM creation, which is a time-consuming operation,
does not block the CPU for a single long interval, allowing for concurrent execution of other
workloads.
