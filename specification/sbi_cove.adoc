...
This operation is used to extend the initial measurement for a TVM for added page contents.
...
struct MeasurementRegisterDescriptor {
       ...
       /*
        * This is the TCG PCR index this measurement maps to, such as [0~16,23] as defined in
...
measurement register. TVMs can read both initial and runtime measurement register
values back.
...
the VMM to finalize the measurement of a TVM (initial). The TSM enforces that the TVM
