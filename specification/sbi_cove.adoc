...
copies the entire VM's state into confidential memory. This is performed as a single operation after 
...
from the NACL shared memory and forwarding it to the TVM.
...
Since now on, the OS/VMM must interact with this TVM via the TSM using the COVH ABI, i.e., resuming the TVM using the `sbi_covh_run_tvm_vcpu()` call.
