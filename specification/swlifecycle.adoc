...
confidential and non-confidential, e.g., during platform initialization.
...
This specification allows two ways of creating a TVM, an implementation must
...
the explicit call to finalize the TVM creation. In the single-step TVM creation, a TVM 
starts executing as a regular VM and requests that the TSM converts it into a TVM. 
TSM creates then a TVM in a single operation by copying the VM to confidential memory.
...
Similarly to the multi-step TVM creation, also in this method, the OS/VMM is involved in
the TVM creation. Specifically, it constructs a regular VM by allocating memory for VM's data,
vCPUs, and creating the G-stage page table configuration. This is the standard process of 
creating non-confidential VMs implemented by existing OS/VMMs. To become a TVM, a VM makes 
the `sbi_covg_promote_to_tvm()` call to the TSM, which uses then the VM state (TVM data, page table
configuration, vCPU state) to recreate the TVM in confidential memory. The promotion of VM to 
TVM must be communicated to the OS/VMM, so that OS/VMM releases the unnecessary resources and 
begins to interact with the TVM in an appropiate way, described in next sections.
