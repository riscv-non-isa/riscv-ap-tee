...
To become a TVM, a VM requests that the TSM changes it into
a TVM using the `sbi_coveg_promote_to_tvm()` call. 
...
When a TVM context is created and intialized by using the `sbi_coveg_promote_to_tvm()`
...
to confidential memory. In deployment model 3, when Smmtt is not available and memory is statically divided
between confidential and non-confidential, dynamic assignment and conversion of memory is not 
...
[NOTE]
====
If Smmtt is not available and memory is memory is statically partitioned into confidential and non-condiential, the TVM shutdown procedure is
simpler. The OS/VMM can reclaim all confidential pages via the `sbi_covh_reclaim_pages()` call, which will 
verify that the non-confidential pages are (no-operation) and (1) verify that each confidential page is not assigned to a TVM, (2) scrub each confidential page.
====
