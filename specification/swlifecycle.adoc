...
confidential and non-confidential, e.g., durich platform initialization.
...
the `sbi_covg_promote_to_tvm()` call to the TSM, which uses then the VM state to recreate the 
TVM in confidential memory. The promotion of VM to TVM must be communicated to the OS/VMM,
so that OS/VMM releases the unnecessary resources and begins to interact with the TVM in 
an appropiate way, described below.
