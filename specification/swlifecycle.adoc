...
creating non-confidential VMs implemented by existing OS/VMMs. To promote a VM to TVM, the 
OS/VMM reflects the `sbi_covh_promote_to_tvm()` call to the TSM. The TSM uses then the VM state 
(VM data, page table configuration, vCPU state) located in non-confidential memory to recreate 
the TVM in confidential memory. After the promotion, the OS/VMM frees the unnecessary VM resources 
and begins to interact with the TVM in an appropiate way, described in next sections.
