...
platform/TSM measurement registers may be stored in the hardware RoTM. The TVM
measurements must be maintained in TSM confidential memory with TVM-bound
lifecycle, effectively making the TSM an extended RoTM for the TVM.
...
All above described TCB elements measurements are added to a remote attestation
...
Systems that cannot rely on remote attestation due to unreliable network
might use local attestation, in which the TSM attests to the integrity of the
TVM and conditions under which it executes using the rules defined in a TVM
attestation payload (TAP), a cryptographically protected file created by the
authorized entity (TVM creator).

[NOTE]
====
The TAP can be provided to the TSM at the time of the TVM creation or runtime.
In the former case, the TSM can enforce that a TVM is allowed to execute and
might provide it with a boot key. In the latter, it can provide the TVM with
continuous access to secrets as part of a licensing model.
====

Local attestation utilizes one or more of the following properties:

. TVM–Platform Binding: The TSM verifies that a TVM creator authorized the TVM
to run on the platform.
. TVM Authorization: The TSM enforces that only TVMs created by a trusted entity
(e.g., a TVM creator) runs on the hardware.

A CoVE-enabled platform supporting the TVM-platform binding must consist of a
mechanism to identify the platform, for example via cryptographic keys exposed
by an implementation-specific mechanism, such as a hardware root of trust with
...
all TCB elements, such as the TSM and other M-mode firmware.
...
A CoVE-enabled platform supporting the TVM authorization must restrict what
TVMs are allowed to run. There must be an implementation-specific mechanism to
provide the TSM with a list of public keys of those authorized to sign TVM
images for execution on the platform. The creator of a TVM must sign the TAP
associated with the TVM with the corresponding private key. During local
attestation, the TSM verifies the signature inside the TAP using the list of
authorized public keys to decide if the TVM is authorized to execute. If the
signature is not valid, the TSM rejects to promote the VM to TVM.

[id=tap_format]
[caption="Figure {counter:image}"]
[title= ": Format of the TVM Attestation Payload (TAP)"]
image::tap_format.png[align=center]

==== TVM Attestation Payload (TAP)
The TAP carries information that allows the TSM to authorize the TVM's execution
on the platform and provision the TVM with TVM-specific secrets.
<<tap_format>> shows the TAP format that consists of five sections: header, lockboxes,
measurements, attributes, and secrets. The last three sections are confidentiality
and integrity protected using a symmetric encryption key, in which a symmetric
key is encrypted inside lockboxes.

[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "TAP Format in CDDL"]
[source,cddl]
----
tap-header = bytes .size 10                           ; Semantic structure applied externally (non-CBOR)

tap-confidential-sections = {
   0 => tap-measurements,
   1 => tap-attributes,
   2 => tap-secrets
}

tap-confidential-sections-encrypted = COSE_Encrypt1   ; The plaintext is defined as tap-confidential-sections

tap = (
   tap-header,
   tap-lockboxes,
   tap-confidential-sections-encrypted
)
----

===== Header
The header section carries information about the size, version, and encoding
of the TAP that are used by a parser to interpret the binary representation of
the TAP. The header must be always 4B aligned, 10 bytes long, and be serialized
in a binary format.

The version field allows future extensions of the TAP format while
preserving backward compatibility. The `encoding` field enables use of
implementation-specific encoding format, for example binary, concise binary object
representation (CBOR), or JavaScript object notation (JSON).
...
Offset | Size | Field
-------+-------+---------------------
0      | 4     | magic constant (ACE0ACE0)
4      | 4     | size (uint32 little-endian)
8      | 1     | version (0–255)
9      | 1     | encoding (0–255)
----
...
the target platforms. Only the TSM running on the target platform within the
expected TCB can access the TSK and thus decrypt the confidential sections.

To construct a TAP, the TVM owner combines the confidential sections into a
single blob and encrypts it with a freshly generated random symmetric key (TSK).
...
Typically, the value stored in a measurement register represents the result of
...
under which a TVM can execute and access the secrets. The TSM implementation
might not support all types of attributes. It must not expose secrets to a TVM,
if any condition fails. If there is an attribute that the TSM does not support,
then the TSM must consider that the condition defined by the attribut failed
verification.
...
firmware_build_time = 1                  ; Firmware build timestamp
firmware_version = 2                     ; BIOS/UEFI version string
tcb_svn = 3                              ; TCB's Secure Version Number (SVN)
secure_boot_enabled = 4                  ; Boolean (1=true, 0=false)
platform_debug_enabled = 5               ; Boolean (1=true, 0=false)
tvm_debug_enabled = 6                    ; Boolean (1=true, 0=false)
max_allowed_uptime = 7                   ; Maximum platform uptime in seconds
...
and a value is a secret, for example, a cryptographic key or an SSH key.
The keys for the key value pairs must be unique.
The TVM can retrieve secrets from the TSM using the `sbi_covg_retrieve_secret()`
ABI.

[NOTE]
====
A common deployment pattern is to protect the confidentiality of the TVM image
at rest by encrypting the VM's disk with a key that the TVM obtains during boot
following successful attestation. In such a case, the TAP would contain a
disk encryption secret that the TVM firmware could retrieve from TSM.
