...
tap-confidential-sections = {
   tap-measurements: tap-measurements,
   tap-attributes: tap-attributes,
   tap-secrets: tap-secrets
}

tap = {
   tap-header: tap-header,
   tap-lockboxes: tap-lockboxes,
   tap-confidential-sections: COSE_Encrypt1
}
----
...
the TAP. The version field allows future extensions of the TAP format while
...
[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "TAP Header Format in CDDL"]
[source,cddl]
----
tap-header-version = 0 / int .ge 0 .le 65535
...
tap-header = {
    magic: h'ACE0ACE0',       ; 4-byte constant
    tap-header-size: int .ge 8 .le 65535,
    tap-header-version: tap-header-version,
    tap-header-encoding: tap-header-encoding
}
----
...
tap-lockbox = {
   tap-lockbox-name          ; Platform identifier, e.g., fingerprint
   tap-lockbox-value         ; Encrypted TSK
}
...
integrity matches all defined reference measurements for a TSM-selected
algorithm. If the `attributes` section is not empty, then the TSM must
additionally verify all attributes before releasing the key.

[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "TAP Measurements Format in CDDL"]
[source,cddl]
----
tap-measurement-pcr-id = int

tap-measurement-algorithm = 1 / 2 / 3 ; 1=sha256, 2=sha384, 3=sha512

tap-measurement-value = bstr

tap-measurement = {
   tap-measurement-pcr-id        ; Measurement register index
   tap-measurement-algorithm     ; Hash algorithm identifier
   tap-measurement-value         ; Measurement value
}
...
[source,cddl]
----
tap-attribute-key = int {
   0 => hardware_serial_number,      ; A unique number identifying hardware
   1 => firmware_build_time,         ; Time at which the firmware was built
   2 => license_expiration,          ; Time at which the license expires
   3 => bios_version,                ; BIOS/UEFI version string
   4 => secure_boot_enabled,         ; Boolean (1=true, 0=false)
   5 => tcb_version,                 ; Trusted Computing Base version
   6 => enclave_debug_allowed,       ; Boolean, e.g., SGX/TDX
   7 => max_allowed_uptime           ; Maximum platform uptime in seconds
}

tap-attribute-operand = int {
   0 => eq,
   1 => ne,
   2 => gt,
   3 => lt,
