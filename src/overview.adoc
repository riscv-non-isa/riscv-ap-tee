...
On processors supporting multiple supervisor domains (using the RISC-V
Supervisor Domains privileged ISA extension), an M-mode root domain security
manager (RDSM) isolates memory, hart execution state and IO resources. The
hosting supervisor domain (OS/VMM) manages the memory, CPU,
...
(Smmpt) is agnostic of the number of supervisor domains. On processors that do
not support multiple supervisor domains where Smmpt is not implemented, a single
confidential  supervisor domain and a single hosting supervisor domain can be
supported using other hardware memory isolation techniques like PMP, G-stage
page tables. See, for example <<dep2>> and <<dep3>>, other deployment models may
also be possible.
...
includes the TSM and hardware) enforces strict confidentiality and integrity
security properties for workloads in this supervisor domain. The Root Domain
Security Manager, also called the " *TSM-driver* ", isolates the Confidential
Supervisor Domain from all other Supervisor domains and other platform
components (non-confidential and confidential). The responsibility of the TSM
is to enforce the security objectives accorded to TEE workloads (TVMs) assigned
to that supervisor domain.
The host VMM is expected to continue to manage the security for non-confidential
...
TSM runs in HS-mode. Systems without Smmpt or the requirement for multiple
supervisor domains can combine the functions of the TSM-driver and TSM in
M-mode, see <<appendix_d>>.
When the supervisor domains privileged ISA is in use, the TSM-driver is in the TCB of all
...
* Creation and initialization of supervisor domains with a configuration
provided by the host to allocate memory and IO devices to the supervisor domain.

* Isolation of memory between supervisor domains - for CoVE, a confidential
supervisor domain is instantiated to host TEE VMs (including the TSM). We
describe *Confidential memory* as memory that is subject to access-control,
confidentiality and integrity suitable per threat model for CoVE components.
For Supervisor Domain memory isolation, the Smmpt extension may be used for
...
* Context switching of the hart supervisor domain id and MPT on TEECALL/TEERET
...
The TSM-driver delegates parts of the TVM management functions to the TSM,
...
using Smmpt or PMP), and may be additionally protected against physical access
...
Smmpt for CoVE). This per-hart execution context is used by the processor to
