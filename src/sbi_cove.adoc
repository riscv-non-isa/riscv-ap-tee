...
[source, C]
-------
struct sbiret sbi_covh_bind_vcpu(unsigned long tvm_guest_id,
                                 unsigned long tvm_vcpu_id);
-------

Binds a TVM vCPU to the current physical CPU.
The vCPU must not already be bound.
When AIA is enabled for the `tvm_guest_id`, restoring interrupt state from the
vCPU's software interrupt file into the first free `1 + guest_per_hart` IMSICs
in confidential memory and marking them as used.

`sbi_covh_run_tvm_vcpu` on a bound vCPU can only be issued from the hart that
the vCPU was bound to.
...
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_PARAM   | `tvm_guest_id` or `tvm_vcpu_id` were invalid.
| SBI_ERR_BAD_RANGE       | There are not enough free IMSICs.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===
...
[source, C]
-------
struct sbiret sbi_covh_unbind_vcpu(unsigned long tvm_guest_id,
                                   unsigned long tvm_vcpu_id);
-------

Unbinds a TVM vCPU from the current physical CPU.
Must be called on the physical CPU to which the vCPU is bound.

If AIA is enabled for the TVM, the TSM reconfigures the guest physical address
translation and issues TLB flushes, making sure that the IMSIC pages are not
reachable from the TVM when `sbi_covh_unbind_vcpu` completes.

The interrupt state is saved to the vCPU's software interrupt file and the
guest interrupt files are free to be reclaimed via `sbi_covh_reclaim_pages()`
or bound to another vCPU via `sbi_covh_bind_vcpu()`.
...
[source, C]
-------
struct sbiret sbi_covh_rebind_vcpu_begin(unsigned long tvm_guest_id,
                                         unsigned long tvm_vcpu_id);
-------

Begins the rebinding process for the specified vCPU to the current physical CPU
and the specified confidential guest interrupt file.

If AIA is enabled for the TVM, the TSM reconfigures the guest physical address
translation and issues TLB flushes, making sure that the IMSIC pages are not
reachable from the TVM when `sbi_covh_rebind_vcpu_begin` completes.
TLB invalidation sequence for the TVM before cloning the old interrupt file
state using `sbi_covi_rebind_vcpu_clone()`.
...
[source, C]
-------
struct sbiret sbi_covh_rebind_vcpu_clone(unsigned long tvm_guest_id,
                                         unsigned long tvm_vcpu_id);
-------

TSM clones the old guest guest state of the specified vCPU. The cloned copy
is maintained in vCPU specific structure visible to TSM only. The host must
make sure to invoke this from the old physical CPU.

This has the same side-effects as calling `sbi_covh_unbind_vcpu`.
...
[source, C]
-------
struct sbiret sbi_covh_rebind_vcpu_end(unsigned long tvm_guest_id,
                                       unsigned long tvm_vcpu_id);
-------

Completes the rebinding process for the specified vCPU from this physical CPU
and its state. Must be called from the same physical CPU as
`sbi_covi_rebind_vcpu_begin()` and after `sbi_covh_rebind_vcpu_clone()`.
