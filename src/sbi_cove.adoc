...
struct tsm_info {
     ...
     * state in sbi_covh_create_tvm() can be computed as
     * (tvm_state_base_bytes + tvm_state_extra_bytes_per_vcpu * vcpu_count
     *  + 4095) / 4096.
     * `0` if the TSM does not support the dynamic memory allocation capability
     * or if it uses reserved memory for TVM state.
     */
    unsigned long tvm_state_base_bytes;
    unsigned long tvm_state_extra_bytes_per_vcpu;
    /*
     ...
     * capability or if the TSM uses `tvm_state_addr` for vCPU state.
     */
...
`tvm_state_addr` is ignored if `tsm_info.tvm_state_base_bytes` and
`tsm_info.tvm_state_extra_bytes_per_vcpu` are both `0`, otherwise
...
pages must be at least the value computed as
`(tsm_info.tvm_state_base_bytes + tsm_info.tvm_state_extra_bytes_per_vcpu
* vcpu_count + 4095) / 4096`.
