...
struct tsm_info {
     ...
     * state in sbi_covh_create_tvm() with vcpu_count = tvm_max_vcpu_vcpus.
     * `0` if the TSM does not support the dynamic memory allocation capability
     * or if it uses reserved memory for TVM state.
     */
     ...
     * capability or if the TSM uses `tvm_state_pages` for vCPU state.
     */
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_ADDRESS | The whole `struct tsm_info` couldn't be written to `tsm_info_address`.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===
...
[#sbi_covh_get_tvm_state_size]
=== Function: COVE Host Get TVM state size (FID unspecified)
[source, C]
-----
struct sbiret sbi_covh_get_tvm_state_size(unsigned long vcpu_count);
-----

Returns the number of 4KiB pages that need to be passed to sbi_covh_create_tvm
for vcpu_count or lower.

The number of pages is never greater than tsm_info.tvm_state_pages.

[#table_sbi_covh_get_tvm_state_size]
.COVE Host Get TSM Info
[cols="2,3", width=90%, align="center", options="header"]
|===
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_PARAM   | The vcpu_count is not supported.
|===
...
[source, C]
-----
struct sbiret sbi_covh_create_tvm(unsigned long tvm_page_directory_addr,
                                  unsigned long tvm_state_addr,
                                  unsigned long vcpu_count);
-----
Creates a confidential TVM using the specified parameters.

`tvm_page_directory_addr` is the base physical address of the 16 KiB
confidential memory region that will be used for the TVM's page directory.
Must be 16 KiB-aligned.

`tvm_state_addr` is ignored if `sbi_covh_get_tvm_state_size(vcpu_count)`
returns `0` on success, otherwise
`tvm_state_addr` is the base physical address of the confidential memory region
to be used to hold the TVM's state, must be page-aligned and the number of
pages must be at least the value returned in `tsm_info.tvm_state_pages` for
`tsm_info.tvm_max_vcpus` of at least `vcpu_count`, as provided by a call to
`sbi_covh_get_tvm_state_size()`.

`vcpu_count` is the minimal number of vCPUs that the TVM must support.
...
| Error code              | Description
| SBI_SUCCESS             | The operation completed successfully.
| SBI_ERR_INVALID_ADDRESS | `tvm_page_directory_addr` or `tvm_state_addr` was invalid.
| SBI_ERR_INVALID_PARAM   | `vcpu_count` was not supported.
| SBI_ERR_FAILED          | The operation failed for unknown reasons.
|===
...
-----
Adds a vCPU with ID `tvm_vcpu_id` to the TVM specified by `tvm_guest_id`.

The `tvm_state_page_addr` is ignored if `tsm_info::tvm_vcpu_state_pages` is
`0`, otherwise `tvm_state_page_addr` must be page-aligned and point to a
confidential memory region used to hold the TVM's vCPU state, and must be
`tsm_info::tvm_vcpu_state_pages` pages in length.
...
| Error code            | Description
| SBI_SUCCESS           | The operation completed successfully.
| SBI_ERR_INVALID_ADDRESS | `tvm_state_page_addr` was invalid.
