...
6+a| The RDSM may address disclosure and tamper attacks via cryptography. When
     ...
     Also see related requirements around ciphertext disclosure when encryption
     is used. +
...
6+^| **Description**
6+a| A privileged host or TVM programs a DMA bus master device to perform DMA
...
6+^| **Mitigations**
6+a| The RDSM must enforce context switch of perf. mon. counters for supervisor
     domains to ensure TEEs get correct PMU info and prevent data leakage due
